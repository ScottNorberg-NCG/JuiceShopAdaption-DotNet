...
public class TestHashing
{
    ...
    [TestMethod]
    public void TestNoSaltWithPrefix()
    {
        var mockKeyStore = new MockKeyStore();
        mockKeyStore.CreateKey("TestKey", 32);

        var toHash = "One ring to find them";

        var hasher1 = new HashingService(mockKeyStore);
        var hashed1 = hasher1.CreateUnsaltedHash(toHash, HashingService.HashAlgorithm.SHA1, true);

        var hasher2 = new HashingService(mockKeyStore);
        var hashed2 = hasher2.CreateUnsaltedHash(toHash, HashingService.HashAlgorithm.SHA1, true);

        Assert.AreEqual(hashed1, hashed2);

        Assert.IsTrue(hashed1.StartsWith("["));
    }

    [TestMethod]
    public void TestNoSaltNoPrefix()
    {
        var mockKeyStore = new MockKeyStore();
        mockKeyStore.CreateKey("TestKey", 32);

        var toHash = "One ring to bring them all, and in the darkness bind them";

        var hasher1 = new HashingService(mockKeyStore);
        var hashed1 = hasher1.CreateUnsaltedHash(toHash, HashingService.HashAlgorithm.SHA2_512, true);

        var hasher2 = new HashingService(mockKeyStore);
        var hashed2 = hasher2.CreateUnsaltedHash(toHash, HashingService.HashAlgorithm.SHA2_512, true);

        Assert.AreEqual(hashed1, hashed2);

        Assert.IsFalse(hashed1.StartsWith("["));
    }
}
